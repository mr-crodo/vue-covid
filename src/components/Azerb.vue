<template>
  <main class="grey lighten-5 pa-2">
    <section>
      <h1 class="display-1">Azerbaijani and Turkey coronavirus stats</h1>
      <p>General statistics of the two fraternal countries on the number of cases and recovered from coronavirus</p>
...
export default {
  ...
  mounted() {
    ...
    if (this.countryInfo !== true) {
      this.axios
          .get('https://corona.lmao.ninja/v2/countries/Azerbaijan,Turkey?yesterday')
          .then(response => { this.continents = response; this.updateStats() })
          .catch(error => { console.error('An API error: ', error) })
      this.axios
          .get('https://corona.lmao.ninja/v2/historical/Azerbaijan,Turkey?lastdays=30')
